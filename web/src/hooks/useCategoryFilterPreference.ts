...
export function useCategoryFilterPreference() {
  const { user } = useAuth()
  ...
  useEffect(() => {
    ...
    if (profile.category_filters.length) setSelectedCategoryIds(profile.category_filters)
  }, [profile, setSelectedCategoryIds])
